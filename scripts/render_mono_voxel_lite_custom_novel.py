...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=Path("/data0/xxy/code/Active-FFGS-streaming/checkpoints/mono_voxel_lite/checkpoints/epoch_8-step_280000.ckpt"),
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/651a7f83ed093001/render_custom_novel139"),
    )
...
def main() -> None:
    ...
    ckpt_meta: dict[str, Any] = {"checkpoint": str(args.checkpoint.resolve()), "contexts": []}

    with torch.no_grad():
        for context_size in [2, 4, 6, 8, 10]:
            raw_context = context_map[context_size]
            context_idx = _clamp_indices(raw_context, max_frame)
            target_idx = [novel_frame]
            ...
            frame_tag = f"t00_f{novel_frame:04d}"
            _to_u8_rgb(pred_rgb[0, 0]).save(ctx_dir / f"{frame_tag}_pred_rgb.png")
            _to_u8_rgb(target["image"][0, 0]).save(ctx_dir / f"{frame_tag}_gt_rgb.png")
            _depth_to_u16(pred_depth[0, 0], args.near, args.far).save(ctx_dir / f"{frame_tag}_pred_depth_u16.png")
            torch.save(pred_depth[0, 0].detach().cpu(), ctx_dir / f"{frame_tag}_pred_depth.pt")

            ckpt_meta["contexts"].append(
                {
                    "context_size": context_size,
                    "context_indices": context_idx,
                    "target_indices": [novel_frame],
                    "context_frame_ids": [frame_ids[i] for i in context_idx],
                    "target_frame_ids": [frame_ids[novel_frame]],
                    "output_dir": str(ctx_dir),
                }
            )

            print(f"ctx{context_size}: context={context_idx}, target={novel_frame}")

    (ckpt_dir / "manifest.json").write_text(json.dumps(ckpt_meta, indent=2), encoding="utf-8")
    top_manifest["checkpoints"].append(ckpt_meta)
    args.output_dir.mkdir(parents=True, exist_ok=True)
    (args.output_dir / "manifest.json").write_text(json.dumps(top_manifest, indent=2), encoding="utf-8")

    print(json.dumps({"output_dir": str(args.output_dir), "checkpoint": ckpt_tag, "novel_frame": novel_frame}, indent=2))
